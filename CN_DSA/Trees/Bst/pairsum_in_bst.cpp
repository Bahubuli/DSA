...
vector<vector<int>> pairsumbst(BtNode<int>* root,int x)
{
    int tcount = countnodes(root);
    ...
    {
        BtNode<int> *ltop = ls.top();
        BtNode<int> *rtop = rs.top();

        if(ltop->data +rtop->data==x)
        {
            b = true;
            ans.push_back({ltop->data, rtop->data});

            BtNode<int> *tp = ls.top();
            ls.pop();
            count++;
        ...
        else if(ltop->data+rtop->data >x)
        {
            BtNode<int> *tp = rs.top();
            rs.pop();
            count++;
