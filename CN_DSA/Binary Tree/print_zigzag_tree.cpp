...
//first element is repeating why ?
//because front was not poped fron the queue after creating head

//why there were only 1 element ans just 1 ll ?
// because when i fixed repeating problem by poping the front
// i forgot to add the front's left and right to the queue

//why was the second element was repeating ?
// i was using if instead of else if

//code looks fine it's not dirty jugad code but still there is
//scope of improvement
vector<Node*> lwisell(BtNode<int>*root)
{
    vector<Node *> ans;
    queue<BtNode<int> *> rest;
    ...
    while(!rest.empty())
    {
        ...
        if (front == NULL && rest.size() > 0)
        {
            ...
            //cout << "first if how many times" << endl;
        }
       else if(front!=NULL)
        {
            if (front->left != NULL)
            ...
            if (front->right != NULL)
                rest.push(front->right);
            curr->next = new Node(front->data);
            curr = curr->next;
            //cout << "second elseif how many times" << endl;
        }
    }
    return ans;
}
...
void printzigzag(BtNode<int>* root)
{
    stack<BtNode<int> *> lr;
    stack<BtNode<int> *> rl;
    rl.push(root);
